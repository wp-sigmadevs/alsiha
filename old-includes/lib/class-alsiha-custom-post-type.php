...
class Alsiha_Custom_Post_Type {
	...
	public function register_post_type() {
		...
		$labels = array_merge(
			...
			array(
				...
				'name'               => sprintf( esc_html_x( '%s', 'post type general name', 'alsiha' ), $plural ),
				/* translators: %s: post type singular name */
				'singular_name'      => sprintf( esc_html_x( '%s', 'post type singular name', 'alsiha' ), $name ),
	...
	public function messages( $messages ) {
		...
		$messages[ $this->post_type_slug ] = array(
			0  => '',
			/* translators: %s: post type name */
			1  => sprintf( esc_html__( '%s updated.', 'alsiha' ), $post_type_name ),
			2  => esc_html__( 'Custom field updated.', 'alsiha' ),
			3  => esc_html__( 'Custom field deleted.', 'alsiha' ),
			/* translators: %s: post type name */
			4  => sprintf( esc_html__( '%s updated.', 'alsiha' ), $post_type_name ),
			/* translators: %s: date and time of the revision */
			5  => isset( $_GET['revision'] ) ? sprintf( esc_html__( '%1$s restored to revision from %2$s', 'alsiha' ), $post_type_name, wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			/* translators: %s: post type name */
			4  => sprintf( esc_html__( '%s published.', 'alsiha' ), $post_type_name ),
			/* translators: %s: post type name */
			4  => sprintf( esc_html__( '%s saved.', 'alsiha' ), $post_type_name ),
			/* translators: %s: post type name */
			4  => sprintf( esc_html__( '%s submitted.', 'alsiha' ), $post_type_name ),
			9  => sprintf(
			...
			/* translators: %s: post type name */
			10 => sprintf( esc_html__( '%s draft updated.', 'alsiha' ), $post_type_name ),
		);
		...
		if ( $post_type_object->publicly_queryable && $this->post_type_slug === $post_type ) {
			...
			$view_link                  = sprintf( ' <a href="%s">%s</a>', esc_url( $permalink ), sprintf( esc_html__( 'View ', 'alsiha' ), $post_type_name ) );
			...
			$preview_link                = sprintf( ' <a target="_blank" href="%s">%s</a>', esc_url( $preview_permalink ), sprintf( esc_html__( 'Preview ', 'alsiha' ), $post_type_name ) );
