...
class Alsiha_Breadcrumbs {
	...
	public function get_breadcrumbs( array $args ) {
		...
		$defaults = array(
			...
			'home_text'                 => sprintf( '<i class="fa fa-home"></i> <span>%s</span>', esc_attr__( 'Startseite', 'alsiha' ) ),
			'cat_archive_prefix'        => esc_attr__( 'Category:&nbsp;', 'alsiha' ),
			'tag_archive_prefix'        => esc_attr__( 'Tag:&nbsp;', 'alsiha' ),
			'search_prefix'             => esc_attr__( 'Search query for:&nbsp;', 'alsiha' ),
			'error_prefix'              => esc_attr__( '404 - Page not found', 'alsiha' ),
		);
	...
	private function start_breadcrumb_rendering() {
		...
		// Woocommerce support.
		// if ( class_exists( 'WooCommerce' ) && ( ( Alsiha_helpers::inside_woocommerce() && is_archive() && ! is_shop() ) || is_cart() || is_checkout() || is_account_page() || Alsiha_Helpers::inside_product_cat() ) ) {
		...
		if ( is_singular() ) {
			...
			} elseif ( ( is_tax() || is_tag() || is_category() || is_date() || is_author() ) && $this->display_post_type_archive && ! Alsiha_helpers::inside_woocommerce() ) {
				$this->render_markup .= $this->get_post_type_archive();
			}
			...
			if ( is_post_type_archive() ) {
				...
				// 404 Page.
				if ( Alsiha_helpers::inside_tribe_event() || ( is_post_type_archive( 'tribe_events' ) || ( Alsiha_helpers::inside_tribe_event() && is_archive() ) ) ) {
	...
	private function get_terms() {
		...
		if ( 'post' == $this->post->post_type ) {
			$taxonomy = 'category';
		} elseif ( 'product' == $this->post->post_type && class_exists( 'WooCommerce' ) && Alsiha_Helpers::inside_woocommerce() ) {
	...
	private function get_woocommerce_page( $linked = true ) {
		...
		if ( is_object( $post_type_object ) && class_exists( 'WooCommerce' ) && ( Alsiha_Helpers::inside_woocommerce() || is_cart() || is_checkout() || is_account_page() ) ) {
