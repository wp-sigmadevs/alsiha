...
if ( ! function_exists( 'alsiha_the_page_title' ) ) {
	...
	function alsiha_the_page_title() {
		...
		if ( is_home() ) {
			$title = get_theme_mod( 'alsiha_pagetitle_blog', __( 'Blog', 'alsiha' ) );
		} elseif ( isset( $_GET['show_term'] ) && 'smartcircle' === $_GET['show_term'] ) {
			$title = __( 'smartCircle', 'alsiha' );
		} elseif ( isset( $_GET['show_term'] ) && 'pedalo' === $_GET['show_term'] ) {
			$title = __( 'Pedalo', 'alsiha' );
		} elseif ( isset( $_GET['show_term'] ) && 'marken' === $_GET['show_term'] ) {
			$title = __( 'Marken', 'alsiha' );
		} elseif ( is_archive() ) {
			$title = get_the_archive_title();
		} elseif ( is_search() ) {
			$title = __( 'Search results for', 'alsiha' ) . ' "' . get_search_query() . '"';
		} elseif ( is_404() ) {
			$title = __( 'Page Not Found', 'alsiha' );
		} else {
			global $post;
...
if ( ! function_exists( 'alsiha_classic_pagination' ) ) {
	...
	function alsiha_classic_pagination() {
		...
		$pagination = Alsiha_Pagination::get_instance();
...
if ( ! function_exists( 'alsiha_comments_meta' ) ) {
	...
	function alsiha_comments_meta() {
		...
		if ( ! post_password_required() && ( comments_open() || 0 !== intval( get_comments_number() ) ) ) {
			$comments_number = get_comments_number_text( esc_html__( 'Click to Comment', 'alsiha' ), esc_html__( '1 Comment', 'alsiha' ), esc_html__( '% Comments', 'alsiha' ) );
...
if ( ! function_exists( 'alsiha_comment_form' ) ) {
	...
	function alsiha_comment_form() {
		...
		$url = '<div class="col-12 col-sm-12 col-md-4">' .
				...
				'<fieldset>' .
				'<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' value="' . esc_attr( $commenter['comment_author_url'] ) . '" placeholder="' . esc_html__( 'Webseite', 'alsiha' ) . '" size="30" />' .
				'</fieldset>' .
				'</div>' .
				'</div>';
		...
		$cookies = '<div class="col-12 col-sm-12 col-md-12">' .
					...
					'<label class="form-check-label" for="wp-comment-cookies-consent">' . esc_html__( 'Meinen Namen, meine E-Mail-Adresse und meine Website in diesem Browser speichern, bis ich das nächste Mal kommentiere', 'alsiha' ) . '</label>' .
					'</fieldset>' .
					'</div>' .
					'</div>' .
					'</div>';
		...
		$comment_field = '<div class="comment-form-comment">' .
						'<fieldset>' .
						'<textarea id="comment" placeholder="' . esc_html_x( 'Kommentar', 'noun', 'alsiha' ) . ( $req ? ' *' : '' ) . '" name="comment" cols="45" rows="8" aria-required="true"></textarea>' .
						'</fieldset>' .
						'</div>';
		...
		$args = apply_filters(
			...
			array(
				...
				'comment_notes_after'  => '',
				'title_reply'          => esc_html__( 'Haben Sie etwas zu sagen?', 'alsiha' ),
				'title_reply_to'       => esc_html__( 'Haben Sie etwas zu sagen?', 'alsiha' ),
				'cancel_reply_link'    => esc_html__( 'Kommentar abbrechen', 'alsiha' ),
				'comment_field'        => $comment_field,
				'label_submit'         => esc_html__( 'Senden', 'alsiha' ),
			...
			<div class="comment-body">
				<div class="comment-media d-flex">
					...
					if ( ! empty( get_avatar( $comment ) ) ) {
						...
						<div class="comment-author vcard d-none d-sm-block">
							...
							echo get_avatar(
								...
								$size    = '128',
								$default = '',
								$alt     = sprintf( '%1$s %2$s', esc_html__( 'Avatar for', 'alsiha' ), get_comment_author() )
							);
							?>
						</div><!-- .comment-author -->
						<?php
					...
					<div class="comment-content">
						...
						// Comment Author link.
						printf(
							wp_kses_post( '<cite class="fn">%s</cite>', 'alsiha' ),
